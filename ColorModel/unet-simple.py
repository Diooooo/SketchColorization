...
class UnetSimple:
    ...
    def train(self, dataset_path, list_files, epochs, learning_rate=0.01, save_index=1, l1_rate=0.1, l2_rate=1):
        ...
        test_condition = data_parser.get_batch_color_hint()
...
if __name__ == '__main__':
    ...
    losses = model.train(dataset_path, list_files, 5000, learning_rate=0.0001,
    ...
    # sess = tf.Session()
    # new_saver = tf.train.import_meta_graph('./checkpoints/unet_simple24/model.ckpt.meta')
    # new_saver.restore(sess, tf.train.latest_checkpoint('./checkpoints/unet_simple24'))
    #
    ...
    #
    # generated = sess.run(logits, feed_dict={inputs: test.item()['input'],
    #                                         condition: test.item()['condition'],
    #                                         is_train: False})
    # sess.close()
    #
    # plt.figure()
