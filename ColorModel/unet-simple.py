...
class UnetSimple:
    ...
    def train(self, dataset_path, list_files, epochs, learning_rate=0.01, save_index=1, l1_rate=0.1, l2_rate=1,
              restore=False, restore_path=None, restart=0):
        ...
        tf.add_to_collection('logits', g_logits)
        ...
        losses = []
        if restore:
            test = np.load('./results/test_data_{}.npy'.format(save_index))
            test_input = test.item()['input']
            test_condition = test.item()['condition']
            test_target = test.item()['target']
        else:
        ...
        with tf.Session(config=config) as sess:
            ...
            # pre-train generator
            for g_epoch in range(restart, epochs):
                for i in range(data_parser.iteration):
                    ...
                    _, loss_g, loss_l1, loss_l2 = sess.run([g_pretrain, g_loss, l1_loss, l2_loss],
                                                           feed_dict={input: batch_input,
                                                                      ...
                                                                      is_training: True},
                                                           options=run_options)
                    loss_g_val, loss_l1_val, loss_l2_val = sess.run([g_loss, l1_loss, l2_loss],
                                                                    feed_dict={input: test_input,
                                                                               target: test_target,
                                                                               condition: test_condition,
                                                                               is_training: False},
                                                                    options=run_options)
                    losses.append([loss_g, loss_l1, loss_l2, loss_g_val, loss_l1_val, loss_l2_val])
                    if i % 10 == 0:
                        print(
                            'Epoch: {}, Iteration: {}/{}, total_loss: {}, l1_loss: {}, l2_loss: {}, '
                            'l1_val_loss: {}, l2_val_loss: {}'.format(
                                g_epoch + 1, i + 1, data_parser.iteration, loss_g, loss_l1, loss_l2, loss_l1_val,
                                loss_l2_val))
                print('*' * 10,
                ...
                if (g_epoch + 1) % 10 == 0:
                    ...
                    output_pre = sess.run(g_logits, feed_dict={input: test_input,
                                                               ...
                                                               condition: test_condition,
                                                               is_training: False})
                    saver.save(sess, './checkpoints/unet_simple{}/model-{}.ckpt'.format(save_index, g_epoch + 1))
                    np.save('./results/predicted_pre_{}-{}.npy'.format(save_index, g_epoch + 1), np.asarray(output_pre))
                    np.save('./logs/train/losses_{}-{}.npy'.format(save_index, g_epoch + 1),
                            np.asarray(losses)[g_epoch * data_parser.iteration:(g_epoch + 1) * data_parser.iteration])
            for _ in range(10):
                print('*' * 10)
...
if __name__ == '__main__':
    ...
    losses = np.load('./logs/train/losses_{}.npy'.format(index))  # start from 10, need concat the first 10 epochs
    losses_10 = np.load('./logs/train/losses_{}-10.npy'.format(index))
    losses = np.concatenate((losses_10, losses), axis=0)
    # model = UnetSimple(resize_shape, batch_size=batch_size)
    #
    # losses = model.train(dataset_path, list_files, 100, learning_rate=0.0001,
    #                      save_index=index, l1_rate=l1_rate, l2_rate=l2_rate, restore=True,
    #                      restore_path='././checkpoints/unet_simple27/model-10.ckpt', restart=10)
    ...
    plt.show()
    #
    # imgs_pre = np.load('./results/predicted_pre_{}-10.npy'.format(index))
    ...
    for i_image in range(2):
        ...
        for i in range(10):
            imgs_pre = np.load('./results/predicted_pre_{}-{}.npy'.format(index, int(10 * (i + 1))))
            img = imgs_pre[i_image]
            img = (1 - img)
            # img = img.astype(np.uint8)
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            # print(img.shape)
